...
type msg =
  ...
  | Set(int); /* This will be our message to set the counter to a specific value */ /* This is a nice quality-of-life addon from Bucklescript, it will generate function names for each constructor name, optional, but nice to cut down on code, this is unused in this example but good to have regardless */
...
let update = model =>
  fun /* These should be simple enough to be self-explanatory, mutate the model based on the message, easy to read and follow */
  | Increment => model + 1
  | Decrement => model - 1
  | Reset => 0
  | Set(v) => v;
